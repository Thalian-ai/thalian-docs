...
- **AI spend estimate shown on AI agent entities.** The entity detail panel for an Okta AI agent now includes an **AI API spend** row in the Agent info section, showing the workspace-level benchmark-priced estimate labeled "Est." The Applications page **AI on your behalf** stat tile similarly shows the estimate below the count when data is available.

- **Compliance page free-plan gate leads with AI governance frameworks.** The gate screen that free-plan users see now previews NIST CSF 2.0 and ISO 42001 controls alongside SOC 2, with sample rows for **PR.AA-01** (identities and credentials) and **A.6.2.6** (AI system operation and monitoring). The call-to-action names all four frameworks and reflects the current rule count (432+).

- **Cross-platform join claim stated on compound findings.** The cross-platform reveal shown after a second integration syncs now uses specialized copy for NHI and AI governance findings, naming the specific pair of systems that produced the insight. The compound-risk hero on the dashboard states the join claim directly rather than describing it generically.

- **Navigation consolidated to a seven-item sidebar.** The left navigation collapses from twelve entries to a seven-item spine: **Overview**, **Findings**, **Inventory**, **Remediation**, **Compliance & posture**, **Integrations**, and **Settings**. Three parent pages now group what used to be separate top-level routes. **Inventory** (`/inventory`) holds **People**, **Non-human**, **Applications**, and **Devices** as tabs, and the **Non-human** tab is always present -- even on a brand-new workspace it shows what it catches: service accounts, tokens, OAuth grants, bots, and AI agents. **Remediation** (`/remediation`) gains a **Simulate** tab for before/after impact analysis. **Compliance & posture** (`/compliance`) folds access reviews, policies, posture, and reports into tabs alongside controls. Every previous URL keeps working through a redirect: `/identities`, `/applications`, and `/devices` land on the matching **Inventory** tab; `/impact-analysis` lands on **Remediation -> Simulate**; and `/access-review`, `/policies`, `/posture`, and `/reports` land on the matching **Compliance & posture** tab. Bookmarks that carry `?tab=`, `?filter=`, or `?framework=` parameters resolve to the right tab and filter, so saved deeplinks and shared URLs continue to work.

- **Cross-entity navigation.** Every identity, device, and application row in the inventory now carries a quick-navigate button that opens the Findings page (`/findings`) pre-filtered to that entity's open findings. On the Applications page the user-count chip links to the Identities page (`/identities`) filtered to users of that application, so you can immediately see who has access without manually searching. The entity detail panel also gains a **View all findings** link in the Findings section header, pointing to the same filtered view. No separate filter setup is required -- clicking the button lands you directly on the right slice.
...
- **Compliance controls with no detection data no longer score as passing.** A compliance control mapped to detection rules that Thalian has not yet evaluated (because the required integration is not connected, or the rule produces no findings for this workspace) previously appeared as green "Passing" in the control table. This inflated the compliance score and gave a false confidence signal for unmonitored attack surfaces. Controls in that state now render as **Not monitored** with a distinct neutral style and are excluded from the framework score calculation. Only controls with at least one evaluated rule count toward the score, so the percentage reflects actual coverage rather than assumed coverage.
...
- **Sanctioned app scope drift detection**: Thalian now detects when a sanctioned application's live OAuth grant has expanded beyond the scope baseline that was approved when the app was sanctioned. The new **Sanctioned app scope drift** finding (high severity, escalates to critical when the new scopes include write-class permissions) fires once per workspace, grouped across all affected apps, so a single finding covers every app with scope drift rather than flooding the finding list. The finding carries a stable key so it persists until every drifted app has been re-attested or revoked -- it does not resolve simply because one app in the group is handled. Two remediation actions are available directly from the finding panel: **Re-attest scope** approves the expanded scopes as the new baseline and writes the updated `allowed_scopes` to the app policy, and **Revoke sanction** marks the app as unauthorized if the new scopes should not have been granted at all. Both actions are audit-logged (`sanction_reattested` / `sanction_revoked`) and the finding self-heals on the next analysis run once the app re-reads as within scope. The finding also surfaces as an agentic queued action in workspaces with agentic remediation enabled, letting you batch-review drifted apps in one pass. Separately, AI governance suppression (which prevents noise from sanctioned apps) now requires an app to be `within_scope` -- apps whose grant exceeds their approved baseline are no longer suppressed, so their other findings remain visible until the scope drift is resolved. The finding maps to SOC 2 **CC6.1**, **CC6.3**, **CC7.2**, **CC8.1**, and **CC9.1**; ISO 27001 **A.5.15**, **A.5.18**, and **A.5.19**; NIST CSF 2.0 **PR.AA-01**, **PR.AA-03**, **PR.AA-05**, and **ID.SC-04**; and ISO 42001 **A.10.3**.

- **Headless API (`/api/v1`)**: Findings and inventory data are now readable programmatically without the web UI, so CI pipelines, scripts, and external tools can consume Thalian directly. A new versioned surface under `/api/v1` accepts the same `thal_*` API keys as the MCP endpoints and returns a stable, curated field subset per collection. `GET /api/v1/findings` returns open findings with severity, category, status, affected entities, and source integrations, filterable by `status` and `severity` with offset-based pagination. `GET /api/v1/inventory` returns identities, applications, or devices (selected by the `type` parameter), each with a versioned field set that omits the raw upstream payload and internal sync columns, so the public contract stays decoupled from the underlying schema. Every request is scoped to the workspace its key belongs to, with no `workspaceId` override, so a key can never read another workspace's data. Read-scope keys (the default) may call these `GET` endpoints; the scope gate denies any method a key's scope does not explicitly grant. Generate a key in **Settings → API Keys**. See the [API Reference](./api-reference.md#headless-api-apiv1) for the full field list and query parameters.

- **Write-scope API keys and `POST /api/v1/scan`**: API keys can now be minted with a `write` scope in addition to the default `read`. A write key unlocks action endpoints on the headless surface, starting with `POST /api/v1/scan`, which triggers a fresh workspace analysis (the same run the **Re-analyze** button performs) so CI pipelines can refresh findings before reading them. The scan endpoint takes no body, derives its workspace from the key, is rate-limited to one trigger per workspace every 60 seconds (`429` with a `retryAfter` outside the window), and records every trigger in the audit log against the calling key for CC8.1 attribution. Read keys remain limited to `GET` and receive `403` on any action endpoint. Minting a key still requires the **Manage workspace** permission, and the chosen scope is captured in the key-creation audit entry. See the [API Reference](./api-reference.md#post-apiv1scan).

- **MCP gateway detection**: Thalian now detects OAuth-connected MCP gateway applications — Composio, Arcade, Pipedream MCP, Klavis, and similar broker platforms that act as a single OAuth grant covering dozens of downstream tool integrations. A behavioral fingerprinting engine scores each OAuth application across six signals: name and homepage match against known MCP gateways (+40 / +15), scope breadth across distinct data categories like Gmail, Drive, Calendar, and Contacts (+20), multi-tool reach when the app brokers access to three or more integration platforms (+25), unverified publisher (+10), and grant burst when multiple users authorized the app within 72 hours (+10). Applications scoring 70 or above are classified as MCP servers. Two new findings surface the risk: **Unsanctioned MCP gateway application** (high, escalates to critical at 5 or more apps) fires when an MCP gateway OAuth application has not been reviewed and approved — one unsanctioned grant can broker AI agent access to many downstream tools simultaneously; **MCP gateway with broad agentic scope** (high) fires when a classified MCP server holds OAuth scopes spanning four or more distinct data categories, indicating an agent can reach across the full collaboration stack. MCP gateway applications are excluded from the existing **Unsanctioned AI tool with sensitive data access** and **AI tool with write access** findings to avoid double-counting the same grant. Both findings map to SOC 2 **CC6.1** and **CC6.3**, NIST CSF 2.0 **ID.AM-05** and **PR.AA-05**, and ISO 42001 **A.10.3** and **A.6.2.2**.

- **Device-side AI agent detection (MDM software inventory)**: AI tool detection now extends from the OAuth surface into MDM-managed device inventory. Thalian's sync handlers for **Fleet**, **Jamf Pro**, **Iru** (formerly Kandji), and **Microsoft Intune** now pull per-device installed software and classify each app against a closed catalog of 30+ AI tools (Cursor, Claude desktop, ChatGPT desktop, Windsurf, Continue, Cody, GitHub Copilot, Codeium, Perplexity, Devin, Ollama, LM Studio, and more). Classified installs land in a new `device_apps` table with `last_seen` lifecycle tracking and a 30-day retention window, then drive the device-side base rule **AI tool installed on managed device** plus three new cross-platform compound rules that join device + identity + OAuth surfaces. **AI tool on device with no corporate OAuth grant** (medium) fires when an AI tool installed on a device has no matching OAuth grant in any connected SaaS — suggesting the user signed in with a personal account, bypassing corporate AI governance and creating vendor cloud egress with no DLP coverage. **AI tool footprint: device + corporate OAuth** (informational) is the happy-path enrichment finding that fires when device install, identity classification, and corporate OAuth all align — surfaced for entity-detail visibility, excluded from workspace risk score. **Local LLM on privileged user device** (high) fires when Ollama or LM Studio runs on a device whose owner has admin access in any IDP — the catastrophic-blast-radius pattern combining vendor-cloud egress with elevated identity. Each finding maps to SOC 2 **CC6.1** and **CC9.1**, NIST CSF 2.0 **PR.AA-01**, **PR.AA-05**, and **ID.AM-01**, and ISO 42001 **A.4.2**, **A.7.3**, and **A.9.2** — so SOC 2 observation evidence and the AI Management System control surface stay tight with the new capability. Intune customers will see a one-time **Software inventory** scope warning on the Integrations page until they reconnect to grant `DeviceManagementApps.Read.All` (added to the standard Microsoft OAuth scope set; a natural extension of the existing `DeviceManagementManagedDevices.Read.All` grant). Fleet, Jamf, and Iru need no re-consent — software inventory access is included in their existing API scope. Reconnect events are audit-logged as `integration_scope_extended` for SOC 2 CC8.1 attribution. The capability requires no rule configuration: connect an MDM, run a sync, and AI installs surface in the next analysis run. See the [Microsoft Intune](./integrations/microsoft-intune.md), [Jamf Pro](./integrations/jamf-pro.md), [Kandji/Iru](./integrations/kandji.md), and [Fleet](./integrations/fleet.md) integration guides for the per-platform sync detail.

- **AI tool detection for Microsoft Entra/M365 OAuth grants**: Thalian's AI-tool exfiltration findings now fire on Microsoft Graph delegated scopes (`Mail.*`, `Files.*`, `Sites.*`, `Calendars.*`, `Contacts.*`) in addition to Google OAuth scopes. The Entra ID sync pulls `oauth2PermissionGrants` and matches Microsoft enterprise apps against the same AI tool catalog used for Google Workspace OAuth tokens (Claude, ChatGPT, Cursor, Gumloop, Microsoft Copilot, Perplexity, n8n, Zapier, and others). Microsoft-shop workspaces will start surfacing AI tool findings on the next sync. For example, a Copilot plugin granted `Files.ReadWrite.All` to twelve users now appears as a critical-severity write-access finding. **No re-authorization required**. The existing `Directory.Read.All` scope already covers `oauth2PermissionGrants` read. Rule IDs and the SOC 2 CC6.1/CC6.3, ISO 27001 A.5.15/A.8.2, NIST CSF 2.0 PR.AA-05/DE.CM, and ISO 42001 A.4.2/A.6.2.6/A.7.3 control mappings are unchanged, so audit evidence continuity is preserved. App-only permissions (`appRoleAssignments`) remain a follow-up; this release covers delegated permission grants. Findings affected: **AI tool has access to corporate data**, **AI tool with write access to email or files**, **Unsanctioned AI tool in widespread use**, and **Terminated employee AI tool still has data access**. See the [Microsoft Entra ID integration guide](./integrations/microsoft-entra-id.md) for details.

- **Link emails across platforms**: When the same person uses different email addresses on different platforms (for example, a corporate `tori@thalian.ai` on Google Workspace and a personal `toriferrante@gmail.com` as the Anthropic billing admin), neither platform reports the connection, so they previously appeared as two separate identities with two separate finding sets. The identity detail panel now has a **Linked emails** section where an admin can mark one or more additional emails as belonging to the same human. The cross-platform dedup layer then merges the identities into a single canonical entity, so cross-platform findings like "this person is admin in Google Workspace and admin on Anthropic billing" surface as a single connected risk instead of two unrelated ones. The action requires the `initiate_remediation` permission, is audit-logged as `identity_aliased` for SOC 2 CC8.1 evidence, and rejects any alias already claimed by another identity in the workspace. Platform-reported aliases (Google secondary emails, Okta `secondEmail`) continue to apply automatically; admin overrides take precedence on conflict. The AI chat context also receives a **Linked identities** block so asking about either email returns the merged view.

- **AI Agent and non-human identity (NHI) governance**: Okta AI Agents are now synced as a first-class identity tier — separate from humans and traditional service accounts. AI agents appear in the Identities page with a dedicated tab and count chip, are excluded from MFA, SSO enforcement, off-hours anomaly, and behavioral baseline rules (agents run 24/7 by design), and are excluded from plan-limit identity counts. The identity detail panel shows a purple **AI Agent** badge plus an **Agent info** section with declared OAuth scopes, client ID, grant type, and owner attribution — with a warning when no human owner is recorded. Admins can also classify identities as AI agents via the Account type dropdown in identity detail, which applies the same NHI-specific governance rules. Two new findings surface NHI risks: **Possible AI agent unclassified** fires when a service account matches AI framework naming patterns (LangChain, CrewAI, Gumloop, n8n, and others) but hasn't been formally classified, prompting an admin to confirm or dismiss; and **AI agent count growing** fires when active agents exceed 20% of the human workforce — a compound-risk signal for uncontrolled NHI proliferation with no offboarding lifecycle. Both findings map to SOC 2 CC6.1/CC6.2 and ISO 27001 A.5.15/A.5.18. Okta requires the `okta.iam.agents.read` scope to pull the AI Agents API; see the [Okta integration guide](./integrations/okta.md) for scope details.

- **Identity classification triage surface**: Classifying an identity as a non-human principal is now a first-class workflow rather than a one-at-a-time dropdown. When Thalian auto-detects that an identity is likely non-human but hasn't been formally classified, its row on the **Identities** page carries an amber **needs review** dot; clicking it opens the detail panel scrolled straight to the **Account type** section. You can now select multiple identities and classify them in one pass with the bulk action bar — each identity is overridden as `user`, `service_account`, `shared_mailbox`, `group`, `bot`, or `ai_agent`, written through the same single audit path (`identity_classified`) one row at a time so the audit hash chain stays intact. The **AI Agents** tab reads **Triage AI Agents** with a warning count whenever there are suspected-but-unclassified agents, and clears to a normal tab once classified. The same classification action is available from **AI chat** ("classify billing-bot@acme.com as a service account") and from the MCP server's new `classify_identity` write-scope tool, so the workflow is reachable from the UI, your AI assistant, and automation alike. Every path enforces the `initiate_remediation` permission server-side. A **Triage machine identities** access-review preset scopes a review to all non-human identities for periodic certification. See the [MCP server documentation](./mcp-server.md) for the `classify_identity` tool.

- **NIST CSF 2.0 compliance framework**: Thalian's detection rules are now mapped to NIST Cybersecurity Framework 2.0, including six controls surfaced on the Compliance page: **PR.AA-01** (privileged access and non-human identity lifecycle governance — the canonical NHI control), **PR.AA-03** (authentication strength across platforms), **PR.AA-05** (access reviews covering users, services, and hardware), **ID.AM-01** (asset inventory completeness), **ID.AM-05** (resource prioritization by criticality), and **DE.CM-03** (personnel and technology activity monitoring). NIST CSF 2.0 explicitly extended PR.AA to cover "users, services, AND hardware" — Thalian uses this extension for AI agent and NHI governance across the platform. NIST CSF 2.0 appears as a third tab on the Compliance page alongside SOC 2 and ISO 27001.

- **ISO 42001 compliance framework**: Thalian now maps detection rules to ISO/IEC 42001:2023, the first international AI management system standard. Seven Annex A controls surface on the Compliance page: **A.4.2** (AI resource documentation), **A.6.2.2** (AI system requirements and specification — the sanctioning gate for AI tools entering the org), **A.6.2.6** (AI system operation and monitoring), **A.6.2.8** (AI system event logs — backed by the immutable audit log), **A.7.3** (data acquired by AI systems via OAuth grants), **A.9.2** (responsible use processes including offboarding lifecycle), and **A.10.3** (third-party AI suppliers). The framework surfaces as a fourth tab on the Compliance page and gives security teams a defensible answer when customers or regulators ask how AI is governed in production. See the [Compliance page documentation](./compliance.md) for the full control coverage table.

- **NHI-only access review scopes**: Access review campaigns now support two new scope filters — **Non-human identities only** (scopes the campaign to all service accounts, bots, and AI agents) and **AI agents only** (narrowest scope, fastest review for agent-heavy workspaces). These scopes fulfill SOC 2 CC6.3 and NIST CSF 2.0 PR.AA-05 review obligations for non-human principals, which previously fell outside the scope of typical human-identity-focused access certifications.
...
- **Executive PDF report includes AI Governance Trajectory**: The **PDF Report** export on the **Reports** page now adds an **AI Governance Trajectory** section to the board-ready **Executive Security Posture Report**. The section tracks three non-human identity signals over time: non-human identities, AI governance findings, and ungoverned or stale keys. Each signal shows its current count plus a 30-day projection derived from your drift snapshot history, so a steadily climbing population reads as *"42 → 61 over 21d (▲ 45%) · ~78 in 30d"*. The section appears only when your workspace has non-human identity data, and its projection matches the **AI Governance Trajectory** chart on the dashboard.

- **Findings group by actor, AI agents first**: The **Findings** page now has a **By actor** grouping mode that organizes your open findings under the identity, application, or device responsible for them, with AI agents and non-human identities (NHIs) listed at the top. When your open findings include any AI agent or NHI, **By actor** becomes the default view so the accounts driving your AI exposure surface first; otherwise the page opens to the flat list. A summary band above the groups counts the AI agents and NHIs with open findings, how many have governance gaps, and how many carry a high or critical finding. Your grouping choice is remembered per workspace in your browser, so the page opens the way you left it, and your selection always takes precedence over the default. Findings that trace to no single actor collapse into a **Systemic** group at the end. Synthetic placeholder addresses that AI providers issue for service accounts (for example, `*-agent.local`) are masked in the actor labels, matching how the **Identities** page already handles them.

- **AI agent and NHI ownership findings now catch null-owner agents**: The cross-provider **AI agent / NHI has no accountable owner** finding previously fired only when an AI agent or non-human identity declared an owner who had since been deprovisioned in the IdP. It now also fires when an NHI carries no owner at all — the null-owner case that is common when an agent, bot, or cloud service principal is provisioned with no creator attribution recorded. Both cases surface as a single grouped finding that lists every affected identity, with each entry annotated by whether its owner left the organization or was never recorded in the first place. This closes a real coverage gap: an agent with no owner is exactly as ungoverned as one whose owner left, and both now appear. The rule remains scoped to non-AI-provider NHIs (Okta AI Agents, Slack bots, cloud service principals) so the per-provider Anthropic and OpenAI owner findings continue to own their populations with no double-counting. Control mappings to SOC 2, ISO 27001, NIST CSF 2.0, and ISO 42001 are unchanged.

- **Cataloged AI bots auto-classify as AI agents**: Slack bots whose names match Thalian's closed catalog of AI tools and AI Automation platforms (Claude, Cursor, Gumloop, Pipedream, Zapier, n8n, and others) now receive the **AI Agent** classification automatically at sync time, instead of waiting for an admin to flip the **Account type** dropdown manually. This means the AI Governance dashboard count, the Identities page **AI Agents** tab, the AI agent suppression of MFA / off-hours / behavioral rules, and the wedge-core compliance evidence all reflect the actual state of your Slack workspace without manual setup. Bots that don't match the catalog (Sentry, generic webhook integrations, your own custom bots) stay as plain service accounts, and bots whose names match AI framework keywords but aren't in the closed catalog continue to surface via the **Possible AI agent unclassified** finding for admin review. Each auto-classification writes an `identity_auto_classified` audit log entry with the catalog match as justification, and the existing **Account type** dropdown remains the way to demote a bot back to plain service account if Thalian got it wrong. The closed catalog also now covers the classic RPA vendors **UiPath**, **Automation Anywhere**, and **Blue Prism**, which carry the same unattended-write risk profile as Power Automate, so their Slack bots auto-classify as AI agents too.

- **NEW badge on recent findings**: Findings that first appeared since you last viewed the **Findings** page carry a teal **NEW** pill. The reference timestamp is per-user, kept in your browser's local storage and scoped to each workspace. It updates after you've spent 30 seconds on the page, so a quick navigation does not reset the clock but a real review marks the badges as seen. On a first-ever visit (or in a new browser), the badge falls back to the same reference the dashboard's *New since last sync* delta card uses (the previous analysis run's drift snapshot). Resolved and dismissed findings never carry the badge. The clock is per-browser, not per-user across devices: if you use two browsers, each has its own clock.

- **Home dashboard now leads with AI agent governance**: The identity card on the dashboard now surfaces AI agents and non-human identities instead of human MFA coverage. The card shows AI agents discovered, NHIs without an owner, OAuth tools acting on behalf of a human, and dormant agents, with a single % governed donut summarizing the population. The CTA routes straight to **Identities** filtered to AI agents (`/identities?type=ai_agent`). MFA coverage has moved to the **Compliance** page as an **Access control evidence** block, mapped to SOC 2 **CC6.1**, NIST CSF 2.0 **PR.AA-01**, and ISO 27001 **A.5.17**, with humans on MFA and admins on MFA shown side by side so auditors and compliance reviewers find it where they expect. Identity insight computation (MFA tallies, dedupe-by-email, NHI counts) now lives in a single shared util used by Home, Compliance, and downstream surfaces, so the numbers stay consistent across the product.

- **AI spend spike detection refined**: The AI Governance spend-spike rule now scales its noise floor to your organization's daily provider spend, so trivial-dollar 2x blips (a workspace going from `$0.12/day` to `$0.26/day` was a "2.1x spike" with `$0.14` of actual movement) no longer fire as findings. Each spike is also attributed to the specific provider workspace where it happened, rather than rolled up into one aggregate finding for the entire provider account, so the affected entity now names the actual workspace (for example, **Anthropic Default Workspace**) rather than the integration. Severity escalates to `high` when a workspace's daily spend hits 5x its trailing 7-day average and stays `medium` for smaller jumps. Applies to Anthropic, OpenAI, and LiteLLM gateway spend. Rule IDs (`ai_governance::anthropic_spend_spike`, `ai_governance::openai_spend_spike`, `ai_governance::litellm_spend_spike`) and their SOC 2 **CC7.2** / NIST CSF 2.0 **DE.CM-03** / ISO 42001 **A.6.2.6**, **A.6.2.8**, **A.10.3** control mappings are unchanged, so monitoring continuity is preserved.

- **GitHub personal access token connection documented**: The [GitHub integration guide](./integrations/github.md) now describes how to connect with a classic personal access token as an alternative to OAuth, including the exact token scopes the sync uses (`read:org` for members and collaborators, `repo` for private repositories and security signals, `admin:org_hook` for org webhooks, `read:audit_log` for Enterprise audit log, and `admin:org` for remediation writes), plus a fine-grained token note for read-only sync. The product has supported this connection method since launch; only OAuth was documented.

- **Slack bot and app integrations surfaced as non-human identities**: Thalian now inventories every bot and app installed in your Slack workspace instead of skipping them. Each appears as a service account on the Identities page, so you can see which automations have access to your channels. A new finding, *"Slack has N active bot integrations, none reviewed,"* flags workspaces with five or more active bot integrations that have never been inventoried. Bots whose names match known AI agent frameworks (Cursor, Gumloop, CrewAI, n8n, and others) are surfaced separately under AI governance for classification. Bot identities never count toward your plan identity limit. Connect or re-sync Slack to populate the inventory. See the [Slack integration guide](./integrations/slack.md).

- **Trend findings are drillable**: Workspace-level trend and aggregate findings now attach the underlying entities so you can act from the finding panel instead of hunting through the Identities, Devices, or Applications pages. Rules updated: *Unapproved application count is growing*, *EDR sensor coverage is declining*, *Unmanaged (BYOD) device count is growing*, *MDM enrollment covers less than 60% of active IDP users*, *Admin MFA coverage is declining over time*, *Service account count growing faster than headcount*, *Applications bypassing SSO is growing over time*, *Legacy authentication protocols detected in Entra sign-in logs*, *Cloud infrastructure admin authenticating via legacy protocols*, *External users represent more than 15% of total identities*, *Potential password spray from one IP*, *Multiple overlapping apps with significant combined spend*. The shadow-IT growth finding flags newly discovered apps as the delta drivers behind the growth rate. Older non-drillable findings auto-resolve atomically via the Superseded tier of the auto-resolve loop on the next analysis run.

- **Compliance Trend chart plots all four frameworks**: The Compliance Trend chart on the Reports page now plots SOC 2, ISO 27001, NIST CSF 2.0, and ISO 42001 scores side-by-side over time, so you can spot which framework is moving and when. Each framework writes its own score column to `drift_snapshots` on every analysis run. Historical gaps appear as breaks in the line until enough snapshots accumulate after the new frameworks land.

- **Exposure trend card uses a consistent 7-day window**: On the dashboard, the **Exposure trend** card's risk score change now compares the current score against the score from seven days ago, matching the new and resolved finding counts shown directly beneath it. Previously the change spanned your full snapshot history while the counts covered only the last seven days, so the two could disagree. The headline score, the sparkline, and the change indicator now all reflect your most recent analysis history.

- **Remediation actions for PingOne (suspend/unsuspend users)**: PingOne findings now expose **Suspend user** and **Unsuspend user** actions inline, calling `PATCH /v1/environments/{envId}/users/{userId}` under the Worker application credentials configured during connect. The Worker role must include write access to the users resource. Other lifecycle actions (`force_password_change`, `revoke_sessions`, `force_mfa_enroll`, `remove_admin_role`) remain admin-console only. See the [PingOne integration guide](./integrations/ping-identity.md) for scope details.

- **Remediation actions for Datadog (disable/enable users)**: Datadog findings now expose **Suspend user** and **Unsuspend user** actions, calling `PATCH /api/v2/users/{userId}` to flip the user's `disabled` attribute. The Application key must have the `user_access_manage` permission. MFA enforcement and session revocation are not exposed by Datadog's API; suspend/unsuspend will terminate active sessions as a side effect. See the [Datadog integration guide](./integrations/datadog.md).

- **Remediation actions for GitLab (remove from group)**: GitLab findings now expose a **Remove from group** action that calls `DELETE /api/v4/groups/{id}/members/{user_id}` under the Group Access Token configured during connect. The user's GitLab account stays active; only the group membership is revoked. Full account suspension still requires Admin API access, which is not available to GATs. See the [GitLab integration guide](./integrations/gitlab.md).

- **Session revocation for Outlook and SharePoint findings**: Outlook and SharePoint findings now support a **Revoke sessions** action that delegates to Microsoft Graph to invalidate the user's refresh tokens. The Outlook and SharePoint integrations intentionally do not surface account-wide lifecycle actions (suspend, force password change, MFA enroll, admin role removal) because those would disable the entire Microsoft 365 identity; perform those from the [Entra ID integration](./integrations/microsoft-entra-id.md) instead.

- **LiteLLM personal-email admin detection**: The `litellm_personal_email_admin` rule now fires when a LiteLLM `proxy_admin` is registered on a personal email domain (such as `@gmail.com` or `@outlook.com`), matching the existing Anthropic and OpenAI versions of the same detection. The LiteLLM sync handler now pulls the user roster via `/user/list` so each identity carries its role, since the existing `/user/daily/activity` endpoint does not expose role data. Older LiteLLM gateways (before v1.50) and master keys without admin scope still sync identities from activity records but skip the role enrichment without failing. Severity is high for `proxy_admin` and medium for other roles on personal email domains. The rule maps to SOC 2 CC6.1 and CC6.2, ISO 27001 A.5.16 and A.5.18, NIST CSF 2.0 PR.AA-01, and ISO 42001 A.6.2.2.
...
- **Compliance control mapping expansion**: The SOC 2, ISO 27001, NIST CSF 2.0, and ISO 42001 control mappings now cover nearly twice as many detection rules as before. Notable additions: access review evidence rules (`access_review::overdue`, `access_review::rubber_stamped`) mapped to CC6.8 so completed access reviews directly contribute to that control's pass/fail; all 15 `ai_governance::*` rules mapped to ISO 42001 and NIST CSF 2.0 for the first time; terminated employee rules expanded across CC6.7 and A.6.5 to include compound multi-platform offboarding failures; cloud IAM stale access and privilege rules (AWS, GCP, Azure) mapped across CC6.5, CC9.1, and A.8.2; and audit trail gap rules added to CC8.1 and A.8.15. All mappings were validated against the live rule set to confirm no broken references.

- **Audit log chain of custody**: Every audit log row now carries `prev_hash`, `row_hash`, and `chain_version` columns forming a per-workspace SHA-256 hash chain. Each new row is cryptographically linked to its predecessor by `sequence_number` order, so any insertion of a forged historical row, any reordering, and any silent deletion of a row from the middle of the log is detectable by walking the chain and verifying link integrity. Workspace creation auto-creates a `audit_chain.genesis` row that seals the pre-cutover row history with a Merkle-style hash root. The chain enhances the evidence quality of SOC 2 CC8.1, ISO 27001 A.8.15, and ISO 42001 A.6.2.8 control mappings. See [Audit log chain of custody](./audit-log-chain-of-custody.md) for the design, threat model, and verification procedure.

- **Audit log chain integrity panel**: The Compliance page audit log tab now shows a live chain integrity panel. On first open it calls the `verify_audit_chain` database function and displays a status badge (**Verified**, **Broken**, or checking), six key chain stats (rows verified, pre-cutover sealed rows, link breaks, hash breaks, chain tip hash, seal root hash), and the specific broken sequence numbers if any integrity failures are found. A **Verify now** button lets you re-run the check on demand. The panel is visible to all roles with audit log access; remediation for any detected breaks happens through your GRC workflow.

- **Audit log JSONL chain export** (Enterprise): A new **Download Chain Export** button on the Compliance page audit log tab downloads the complete audit log as a JSONL file with a chain-metadata header (`thalian-audit-export/1` schema). The header contains the genesis row hash, chain tip hash, pre-cutover seal root, cutover timestamp, and the SHA-256 of the public Python verifier so an auditor can confirm they have the right tool version. Rows appear oldest-first with `row_hash` and `prev_hash` included so the chain can be reconstructed locally. Designed for offline verification with `verify-audit-export.py` in [thalian-public](https://github.com/thalian-ai/thalian). Every download is recorded in the audit log. Requires the Enterprise plan.

- **Workspace AI memory retention**: The `workspace_ai_memory` table (persistent facts the AI observes about your organization across chat sessions) is now subject to your plan's data retention period. Entries older than your retention window are deleted by the daily retention cron, matching the existing handling for chat history (`ai_messages`). Workspace deletion still removes all entries via cascade, and all writes and deletions remain audit-logged as `ai_memory_saved` and `ai_memory_deleted` actions. See [AI Transparency](./ai-transparency.md) for the data class details.

### Fixes

- **Actor badges distinguish AI agents from service accounts**: In the **Findings** page **By actor** view, the badge on each non-human identity now reflects what the identity actually *is*, not the category of finding attached to it. Previously the view inferred "AI Agent" from the presence of an AI governance finding, so a human admin or a plain service account that happened to carry an AI governance finding was mislabeled **AI Agent**. The badge now reads the authoritative identity classification synced from your IdP — `user_type_detail` and `identity_type` — so true AI agents show the purple **AI Agent** badge while other non-human identities show an amber **Service account** badge, with a `· review` annotation when an account matches an AI framework naming pattern but hasn't been formally classified. Humans render as **User**. The summary band above the groups counts all non-human identities (AI agents and service accounts) with open findings rather than only the AI governance subset.

- **Auto-resolve no longer leaves findings stuck on the `applications`, `audit_events`, or `entitlements` source labels**: The 2-miss auto-resolve path runs an `isPlatformHealthy()` health check against every platform in a finding's `source_integrations` list. Generic source labels like `identity_provider` and `mdm` get expanded to the underlying platform set, but three rule-internal data-source labels (`applications`, `audit_events`, `entitlements`) name the underlying table a rule reads, not a platform category, so they were missing from the alias map. The gate returned `false` for those labels, the `every()` check failed, and findings stayed open indefinitely even when every connected integration was syncing healthy. A sentinel value in the alias map now resolves those labels as healthy whenever any connected integration is healthy, while real platform entries in the same list still gate independently. Stuck findings such as Zoom and Claude OAuth-grant findings auto-resolve atomically over the next two analysis cycles.

- **GitHub OAuth and personal access token connections work again**: GitHub's REST API rejects requests without a `User-Agent` header with a silent `403 Forbidden`. This broke two flows. OAuth signup: the `/user/orgs` lookup in the OAuth callback returned 403, `org_login` came back `null`, and the callback rejected with `github_no_org`. PAT validator: the token verifier read the 403 as "token invalid" and refused to save credentials. All five GitHub API call sites now send a `User-Agent: Thalian-IT-Intelligence` header. The PAT validator also passes through 403 responses on the broader `/user` endpoint, so fine-grained personal access tokens scoped to a specific organization (which cannot read `/user`) are accepted as long as the org-scoped calls succeed.

- **Aged-OAuth-grant findings no longer fire on platforms without per-grant usage telemetry**: The `access::entitlement_never_used` rule was firing on aged OAuth grants from Zoom, Claude (Anthropic), and similar providers as "OAuth grant aged 30+ days, no usage signal," but these platforms do not expose per-grant last-used data through their APIs. With no real signal to anchor on, the finding was noise. `google_oauth_token` joins the rule's `NO_USAGE_TELEMETRY_SOURCES` filter alongside other no-telemetry sources, and the dead OAuth-no-telemetry branch in the rule body has been removed. The rule still fires on platforms that do expose per-grant last-used data. A `detection_calibration_change` entry is recorded in the audit log for SOC 2 CC8.1 evidence on the calibration. Existing noise findings auto-resolve on the next analysis run via the Superseded tier of the auto-resolve loop.

- **"Users appearing in SaaS tools without IDP provisioning" finding no longer flags installed bots as ghost users**: This drift finding counted every non-IDP-matched identity created in the last 60 days as an unprovisioned human, but installed Slack bots and apps (Claude, Sentry, Pipedream, custom organization bots) are non-human identities that legitimately have no IDP account — they were being misclassified as ghost users. The rule now excludes `identity_type='service_account'` from both the ghost-creation count and the current-active-ghost count. The finding description also now names the connected IDP (for example, "without a matching account in Google Workspace") instead of saying "any connected IDP," so the platform chips on the finding card match the prose. The finding auto-resolves for workspaces whose only previously-counted ghosts were bots.

- **License overrides no longer auto-classify apps as Approved**: Uploading a contract for an app (which creates a license override row used for cost, renewal date, and tier metadata) was pinning that app into the **Approved** tab on the **Applications** page regardless of policy state. Clicking **Revert** had no visible effect because Revert only clears the policy row, not the license override. License data is contract and cost tracking, not a policy approval, so an app with a license override now falls through to **Unsanctioned** unless it is explicitly sanctioned, SSO-provisioned, or stamped sanctioned by sync data. Contract data is preserved and continues to drive the **Source**, **Monthly Cost**, **Tier**, and **Renewal** columns.

- **Compliance PDF export now includes all controls**: The PDF download on the Compliance page was capturing only the score summary (compliance percentage, passing count, failing count) and leaving the controls table blank. The export now produces a complete report including the full controls table with status badges and open finding counts, regardless of any search or filter currently applied in the UI.

- **AI provider integrations now validate admin keys at save time**: Anthropic, OpenAI, and LiteLLM connect attempts were accepting any non-empty key string and only surfacing the error on the first sync, typically as a 401 from the provider's organization usage endpoint. The connect modal now probes the same endpoint the sync handler uses and rejects the credential immediately with a specific error. The most common cause is pasting a regular API key (such as `sk-ant-api03-...`) instead of an Admin API Key (`sk-ant-admin01-...`); the new error message tells you which type you need and where to create it. The Cloudflare credential probe timeout was also tightened from 30s to 10s across every integration validator so a slow upstream cannot consume the full request budget, and the shared SSRF blocklist was expanded to cover Alibaba Cloud, Oracle Cloud, IPv6 link-local, and several AWS / GCP metadata aliases.

- **Per-app findings now surface on the affected identity's card**: OAuth app findings (`oauth_scope_creep`, `ai_tool_data_access`, `ai_tool_write_access`, `broad_oauth_write_access`) and never-used entitlement findings store affected users in the `affected_entities` payload rather than via a direct identity FK column. The identity page filter was skipping these findings entirely because it required the FK to be set before consulting the payload. The filter now reads user emails out of `affected_entities` directly, so asking "what findings affect alice@company.com?" returns app-attributed findings she appears in. Two rules (`oauth_scope_creep` and `access::entitlement_never_used`) now also populate the FK column when exactly one identity resolves from the user list, so single-user app findings attribute to their identity without changing the finding key or type. The Applications page now indexes grouped findings by the UUID of each member app so per-app finding lists are complete for grouped-finding types.

- **Disconnecting and removing an integration no longer leaves zombie findings**: Findings emitted by 185 detection-rule call sites tagged their data source with a category alias (`mdm`, `identity_provider`, `oauth`, `endpoint_manager`, `hr`, `cloud`, ...) instead of the actual platform ID (`fleet`, `okta`, etc.). When the only platform in a category was removed — for example, disconnecting and removing Fleet from a Fleet-only MDM environment — the cleanup query in `remove-integration.js` (`source_integrations @> [platform_id]`) never matched those alias-encoded rows, and the 2-miss auto-resolve path in `analyze.js` couldn't fire either because no platform in the category was reporting "healthy" anymore. The findings persisted indefinitely with `data_source: "mdm"` and no live integration backing them. Two fixes ship together. First, every finding now passes through a single normalization chokepoint in `analyze.js` before persisting — category aliases get expanded to the actual connected platform IDs via a shared `normalizeSourceIntegrations()` helper (entity-first when the affected entity has a known integration, category-wide otherwise), and unresolvable aliases are dropped entirely. New findings always carry real platform IDs in `source_integrations`. Second, the integration removal flow now also sweeps legacy alias-encoded rows, but only when removing the platform leaves its category with zero connected platforms — so existing alias findings legitimately backed by another connected platform are preserved. The sweep is recorded in the audit log as `orphaned_aliases_swept` for SOC 2 CC8.1 attribution. Rule authors can continue to use category aliases in `source_integrations` for readability — the chokepoint normalization handles resolution.

- **Notification deep-links resolve to `/findings`**: Outbound Slack, Microsoft Teams, email, and webhook alerts built deep-links to a `/risks` route that no longer exists (it was renamed to `/findings`), so clicking through dropped you on the dashboard catch-all instead of the specific finding. URLs now build through a centralized `appUrl()` / `findingUrl()` pair driven by the `SITE_URL` environment variable, and per-finding deep-links carry the `?finding={uuid}` query param so the detail panel opens directly to the finding being notified.
