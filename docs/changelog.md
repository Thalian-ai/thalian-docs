...
- **NEW badge on recent findings**: Findings that first appeared in the most recent analysis run now carry a teal **NEW** pill on the **Findings** page. The badge clears automatically once the next analysis run completes, so it always reflects "since the last sync" without a manual dismiss step. The reference timestamp matches the dashboard's *New since last sync* delta card (the previous drift snapshot's `captured_at`), so both surfaces agree on which findings are new. Resolved and dismissed findings never carry the badge.
